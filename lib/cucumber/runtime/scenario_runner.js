    ...
    run: function run(callback) {
      ...
      eventBroadcaster.broadcastAroundEvent(
        ...
        function (callback) {
          ...
          var MAX_RETRIES = isNaN(process.env.CUCUMBER_RETRIES)?5:parseInt(process.env.CUCUMBER_RETRIES);
          
          (new Array(MAX_RETRIES+1).fill(0)).reduce((prom, dontCare, retryCnt) => {
            return prom.catch(() => {
              return Promise((resolve, reject) => {
                 self.runBeforeHooks(function () {
                  self.runSteps(function() {
                    self.runAfterHooks(function(){
                      self.broadcastScenarioResult((scenarioResult) => {
                            if (scenarioResult.getStatus() ===  Cucumber.Status.FAILED) {
                              console.log('Retrying Scenario for the:', retryCnt, ' time');
                              return reject(scenarioResult);
                            }
                            return resolve(scenarioResult);
                      });
                    });
                  });
                });
